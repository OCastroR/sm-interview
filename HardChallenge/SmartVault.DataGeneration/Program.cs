...
namespace SmartVault.DataGeneration
{
    partial class Program
    {
        static void Main(string[] args)
        {
            ...
            var databaseInitializer = new DatabaseIntializer(connectionString);
            var accountRepository = new AccountRepository(connectionString);
            var documentRespository = new DocumentRepository(connectionString);
            var userRespository = new UserRepository(connectionString);

            databaseInitializer.CreateDatabaseTables(files);
            int accountCount = accountRepository.BulkInsert();
            int userCount = userRespository.BulkInsert();
            int documentCount = documentRespository.BulkInsert();
